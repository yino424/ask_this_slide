...
Tiny Chrome extension for turning the slide or video frame you are watching into bilingual study notes. Built for local use, with your own OpenAI API key. 🎓✨
...
```text
反向传播 -> Backpropagation: 神经网络根据误差从输出层向前更新参数的方法。
損失関数 -> Loss Function: 用来衡量模型预测和真实结果之间差距的函数。
```
